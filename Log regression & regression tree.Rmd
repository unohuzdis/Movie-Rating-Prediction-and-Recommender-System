...
```
MSE is 0.4282526 so this tree predicts rating within .65 of the true response. Which is not bad.
...
```
MSE is 51034.16 so this tree predicts rank within 225.9 of the true response. Not very good
...
```
MSE for revenue is 6791.951 so this tree predicts revenue within 82.41 of the true response.This is not very good but given the large variation of the revenue data its hard to tell
if this is bad.
...
```
MSE for runtime is 327.63 so this tree predicts runtime to within 18.1 minutes. This is not a bad prediction of the runtime
...
```
MSE for runtime is 19079568696 so this tree predicts votes to within 138128.811 votes. Even with the large variance in votes data this prediction is quite bad and shows that a regression tree likely does not work for this response variable
...
```
MSE for metascore is 152.9769 so this tree predicts metascore within 12.37 points. While this might not be the best model for predicting metascore it is not bad.
